...
class Dataset(data.Dataset):
    ...
    def __init__(self, root, data_file_name, vocab, transform=None, return_target=True):
        ...
        self.exist_data = dict()
        idx = 0
        for id in range(len(self.data)):
            candidate_asin = data[id]['candidate']
            candidate_img_name = candidate_asin + '.jpg'
            candidate_img_path = os.path.join(self.root, candidate_img_name)
            target_asin = data[id]['target']
            target_img_name = target_asin + '.jpg'
            target_img_path = os.path.join(self.root, target_img_name)
            if os.path.exists(candidate_img_path) and os.path.exists(target_img_path):
                self.exist_data[idx] = {'candidate':candidate_asin, 'target':target_asin}
                idx += 1
            else:
                print('[ NO EXIST ] candidate: %s / target: %s'%(candidate_asin, target_asin))
        self.ids = range(len(self.exist_data))
        self.vocab = vocab
